  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        if (res.data.success) {
          const igFollowServices = res.data.services.filter(s => 
            s.platform === 'Instagram' && 
            s.isActive !== false && 
            (s.category === 'Tăng Theo Dõi' || s.category === 'Tăng Follower')
          );
          setServices(igFollowServices.map(s => ({
            id: s._id,
            name: s.name,
          ...
          if (igFollowServices.length > 0) {
            setSelectedServer(igFollowServices[0]._id);
          }
        }
      } catch (err) {
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className={`${activeTab === 'create' ? 'xl:col-span-2' : 'xl:col-span-3'} space-y-6`}>
          ...
          {activeTab === 'create' ? (
            ...
            <ServiceOrderList serviceType="instagram_follow" />
          )}
