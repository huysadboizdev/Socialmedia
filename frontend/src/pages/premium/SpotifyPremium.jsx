  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        if (res.data.success) {
          const spotifyServices = res.data.services.filter(s => s.platform === 'Spotify' && s.isActive !== false);
          setServices(spotifyServices);
          if (spotifyServices.length > 0) {
            setSelectedService(spotifyServices[0]);
          }
        }
      } catch (err) {
        ...
        <div className="xl:col-span-2 space-y-6">
          <div className="bg-white dark:bg-slate-900 rounded-lg p-6 shadow-sm border border-slate-100 dark:border-slate-800">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100 dark:border-slate-800">
              <img src={spotifyIcon} alt="Spotify Premium" className="w-8 h-8 rounded-full object-contain" />
              <span className="font-bold text-lg text-slate-800 dark:text-slate-100">Dịch vụ Spotify Premium</span>
            </div>
              ...
              <div className="space-y-3">
                {loading ? (
                  <div className="flex items-center gap-2 py-4 text-slate-400">
                    <span className="material-symbols-outlined animate-spin text-green-500">sync</span>
                      ...
                      className={`p-4 border rounded-xl cursor-pointer transition-all flex items-center justify-between group ${
                        service.isMaintenance 
                          ...
                          : selectedService?._id === service._id 
                            ...
                            : 'border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50 hover:border-green-300'
                      }`}
                    >
                      <div className="flex flex-col gap-1">
                        <span className="font-bold text-slate-700 dark:text-slate-200 group-hover:text-green-600 dark:group-hover:text-green-400">
                          {service.name}
                        </span>
                        {service.description && (
                      ...
                      <span className={`px-4 py-1.5 rounded-full text-sm font-bold shadow-sm ${
                        selectedService?._id === service._id && !service.isMaintenance
                          ...
                          : 'bg-white dark:bg-slate-700 text-green-600 dark:text-green-400 border border-green-100 dark:border-green-900/30'
                      }`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
              <div>
                ...
                <input
                  ...
                  className="w-full p-3 border border-slate-200 dark:border-slate-700 rounded bg-white dark:bg-slate-800 outline-none focus:ring-2 focus:ring-green-500 transition-all font-medium text-sm"
            ...
            <div className="mb-8">
              ...
              <textarea
                ...
                value={formData.note}
                onChange={handleInputChange}
                className="w-full p-3 border border-slate-200 dark:border-slate-700 rounded bg-white dark:bg-slate-800 outline-none focus:ring-2 focus:ring-green-500 transition-all resize-none"
                placeholder="Ví dụ: Nâng cấp gói 1 năm..."
                disabled={selectedService?.isMaintenance}
              ></textarea>
            </div>
            ...
            <div className="mb-8 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800">
               <div className="mb-4">
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={handleInputChange}
                    className={`w-full p-3 border rounded bg-white dark:bg-slate-800 outline-none focus:ring-2 focus:ring-green-500 transition-all ${
                  ...
                  {isValidating && (
                    <div className="absolute right-3 top-1/2 -translate-y-1/2">
                      <span className="material-symbols-outlined animate-spin text-green-500 text-sm">sync</span>
                    </div>
                  )}
                </div>
              ...
              <div className="pt-4 border-t border-slate-200 dark:border-slate-700 text-center">
                <div className="text-xl font-bold mb-1 text-slate-800 dark:text-slate-100">
                  Tổng thanh toán: <span className="text-green-600 dark:text-green-400">
                    {hasDiscount ? (
                      <>
            ...
            <button 
              ...
              disabled={selectedService?.isMaintenance}
              className="w-full py-4 bg-green-500 hover:bg-green-600 text-white font-bold rounded-lg transition-colors flex items-center justify-center gap-2 text-lg shadow-lg shadow-green-200 dark:shadow-green-900/20 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Nâng cấp Spotify Premium ngay
            </button>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-lg p-6 shadow-sm border border-slate-100 dark:border-slate-800">
            <div className="flex items-center gap-2 mb-6 pb-4 border-b border-slate-100 dark:border-slate-800">
              ...
              <span className="font-bold text-lg text-slate-800 dark:text-slate-100">Xem trước giao diện Spotify Premium</span>
            </div>
            
            <div className="flex justify-center">
              <div className="group relative overflow-hidden rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 max-w-md">
                <img 
                  src={spotifyPreview} 
                  alt="Spotify Preview" 
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <span className="text-white font-bold text-sm tracking-widest uppercase">Spotify Premium</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="xl:col-span-1 space-y-6">
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-lg p-5 shadow-sm">
            <h3 className="font-bold text-slate-700 dark:text-slate-200 mb-4 flex items-center gap-2">
              <span className="material-symbols-outlined text-green-500">priority_high</span>
              Lưu ý khi nâng cấp
            </h3>
            <div className="space-y-4 text-xs text-slate-600 dark:text-slate-400">
              <p>• Cần nhập Email chính xác để nhận invite Family.</p>
              <p>• Nhập thông tin liên hệ chính xác để Admin có thể hỗ trợ nâng cấp.</p>
              <p>• Nếu nâng cấp chính chủ, cần thoát Family hiện tại (nếu có).</p>
              <p>• Thời gian hoàn thành từ 5-20 phút.</p>
              <p>• Bảo hành full thời gian đăng ký.</p>
            </div>
          </div>
        </div>
      </div>
