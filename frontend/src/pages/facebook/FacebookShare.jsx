  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        if (res.data.success) {
          const fbShareServices = res.data.services.filter(s => 
            s.platform === 'Facebook' && 
            s.isActive !== false && 
            (s.category === 'Tăng Share' || s.category === 'Share Bài Viết')
          );
          setServices(fbShareServices.map(s => ({
            id: s._id,
            name: s.name,
          ...
          if (fbShareServices.length > 0) {
            setSelectedServer(fbShareServices[0]._id);
          }
        }
      } catch (err) {
          ...
          {activeTab === 'create' ? (
            ...
            {/* Total Footer */}
              <div className="bg-purple-600 text-white p-4 rounded-lg mb-4 text-center">
                ...
                <div className="font-medium opacity-90 text-sm italic">
            ...
            <ServiceOrderList serviceType="facebook_share" />
          )}
        </div>
