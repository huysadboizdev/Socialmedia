  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        if (res.data.success) {
          const tiktokLikeServices = res.data.services.filter(s => 
            s.platform === 'TikTok' && 
            s.isActive !== false && 
            (s.category === 'Tăng Like' || s.category === 'Like Bài Viết')
          );
          setServices(tiktokLikeServices.map(s => ({
            id: s._id,
            name: s.name,
          ...
          if (tiktokLikeServices.length > 0) {
            setSelectedServer(tiktokLikeServices[0]._id);
          }
        }
      } catch (err) {
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className={`${activeTab === 'create' ? 'xl:col-span-2' : 'xl:col-span-3'} space-y-6`}>
          ...
          {activeTab === 'create' ? (
            ...
            <ServiceOrderList serviceType="tiktok_like" />
          )}
        </div>
