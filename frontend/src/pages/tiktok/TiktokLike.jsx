...
const TiktokLike = () => {
  ...
  const servers = [
    { id: 'sv1', name: 'Sv1', label: 'Tim Việt | ⚡ Nhanh ⚡ | Bảo Hành 15 Ngày', price: 14.5, status: 'Hoạt động', color: 'orange' },
    { id: 'sv2', name: 'Sv2', label: 'Tim Việt | ⚡ Nhanh ⚡ | Top 1 Rẻ Nhất', price: 4.3, status: 'Hoạt động', color: 'orange' },
    { id: 'sv3', name: 'Sv3', label: 'Tim Tây | ⚡ Nhanh ⚡ | Cam Kết Không Tụt', price: 3.7, status: 'Hoạt động', color: 'orange' },
    { id: 'sv4', name: 'Sv4', label: 'Tim Tây | ⚡ Nhanh ⚡ | Bảo Hành 15 Ngày', price: 2.5, status: 'Hoạt động', color: 'orange' },
    { id: 'sv5', name: 'Sv5', label: 'Tim Tây | ⚡ Nhanh ⚡ | Ổn Nhất Hiện Tại', price: 1.5, status: 'Hoạt động', color: 'orange' },
    { id: 'sv6', name: 'Sv6', label: 'Tim Tây | Tốc Độ Ổn | Giá Rẻ Nhất Vn', price: 0.39, status: 'Hoạt động', color: 'orange' },
  ];
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 space-y-6">
          <div className="flex gap-2 text-white">
            <button
               // The screenshot uses a solid purple bar for "Khởi Tạo Đơn" which implies active tab
              onClick={() => setActiveTab('create')}
              className={`flex-1 py-3 px-4 rounded font-bold transition-all flex items-center justify-center gap-2 ${
                activeTab === 'create'
                  ? 'bg-[#6f42c1] shadow-lg shadow-purple-500/30' // Custom purple to match image
                  : 'bg-slate-400 hover:bg-slate-500'
              }`}
            >
              <span className="material-symbols-outlined">shopping_cart</span> Khởi Tạo Đơn
            </button>
            <button
              onClick={() => setActiveTab('list')}
               // The "Danh Sách Đơn" in screenshot is just text link-like
              className={`flex-1 py-3 px-4 rounded font-bold transition-all flex items-center justify-center gap-2 ${
                activeTab === 'list'
                  ? 'bg-[#6f42c1] shadow-lg shadow-purple-500/30'
                  : 'bg-transparent text-[#6f42c1] hover:bg-purple-50'
              }`}
            >
              <span className="material-symbols-outlined">history</span> Danh Sách Đơn
            </button>
          </div>
                ...
                {servers.map((server) => (
                  <div key={server.id} className="group relative">
                    ...
                    >
                      ...
                      <span className="font-bold text-white px-2.5 py-1 rounded bg-[#f97316] text-xs shadow-md shadow-orange-500/20">
                        {server.name}
                      </span>
                      
                      <span className="font-semibold text-slate-700 dark:text-slate-300 text-sm flex-1">
                        {server.label.split('|').map((part, index) => (
