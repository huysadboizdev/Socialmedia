  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        if (res.data.success) {
          const tiktokShareServices = res.data.services.filter(s => 
            s.platform === 'TikTok' && 
            s.isActive !== false && 
            (s.category === 'Tăng Share' || s.category === 'Share Bài Viết')
          );
          setServices(tiktokShareServices.map(s => ({
            id: s._id,
            name: s.name,
          ...
          if (tiktokShareServices.length > 0) {
            setSelectedServer(tiktokShareServices[0]._id);
          }
        }
      } catch (err) {
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className={`${activeTab === 'create' ? 'xl:col-span-2' : 'xl:col-span-3'} space-y-6`}>
          ...
          {activeTab === 'create' ? (
            ...
            <ServiceOrderList serviceType="tiktok_share" />
          )}
