...
const TiktokFollow = () => {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        if (res.data.success) {
          const tiktokFollowServices = res.data.services.filter(s => 
            s.platform === 'TikTok' && 
            s.isActive !== false && 
            (s.category === 'Tăng Theo Dõi' || s.category === 'Tăng Follower')
          );
          setServices(tiktokFollowServices.map(s => ({
            id: s._id,
          ...
          if (tiktokFollowServices.length > 0) {
            setSelectedServer(tiktokFollowServices[0]._id);
          }
        }
      } catch (err) {
  ...
  const handleSubmit = async () => {
    if (!formData.link) {
      toast.error('Vui lòng nhập Link profile');
      return;
    }
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className={`${activeTab === 'create' ? 'xl:col-span-2' : 'xl:col-span-3'} space-y-6`}>
          ...
          {activeTab === 'create' ? (
            ...
            <ServiceOrderList serviceType="tiktok_follow" />
          )}
