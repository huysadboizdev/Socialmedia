...
const AdminNotificationDropdown = () => {
  ...
  const fetchNotifications = async (isBackground = false) => {
    try {
      ...
      const res = await axios.get(`${API_URL}/api/admin/notifications`, {
        ...
        prevUnreadCountRef.current = res.data.unreadCount;
      }
    } catch (error) {
  ...
  useEffect(() => {
    fetchNotifications();
    // Poll for new notifications every 3 seconds for pseudo real-time
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-slate-100 dark:border-slate-800 z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          <div className="p-3 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50 backdrop-blur-sm">
            <h3 className="font-semibold text-sm text-slate-800 dark:text-slate-200">Thông báo Admin</h3>
            {unreadCount > 0 && (
                <button 
